...
<div class="content-wrapper">
        ...
        <h1> Laporan Penjualan</h1>
            <ol class="breadcrumb">
              ...
              <li><a href="?pg=lappjsales&act=view"><i class="fa fa-dashboard"></i> Laporan Penjualan</a></li>
             </ol>
        </section>

<section class="content"> <br> <br> <br>
  <div class="row">
    <form action="?pg=lappjsales&act=cek" method="POST">
    <div class="col-md-2">
      ...
      <div class="col-md-2">
          ...
          <select name="sales" class="form-control">
            <?php
              $qsls = mysql_query("SELECT * FROM users WHERE level_user = 'sales' and users.id_user='$_SESSION[sess_id]' ORDER BY nama ASC");
              while ($dsls = mysql_fetch_array($qsls)) {
            ?>
            <option value="<?php echo $dsls['id_user']; ?>"><?php echo $dsls['nama']; ?></option>
            <input type="hidden" name="sales" value="<?php echo $dsls['id_user']; ?>">
            <?php } ?>
          </select>
          </div>
      </div>
...
AND '$_POST[tglpenjualanak]' AND penjualan.metode_penjualan = '$_POST[metode_penjualan]' AND users.id_user = '$_SESSION[sess_id]'
ORDER BY penjualan.nopenjualan ASC");
                      } else {
                        $tampil=mysql_query("SELECT
penjualan_detail.itemterjual,
...
WHERE
  penjualan.tglpenjualan BETWEEN '$_POST[tglpenjualanaw]'
AND '$_POST[tglpenjualanak]' and users.id_user='$_SESSION[sess_id]'
ORDER BY penjualan.nopenjualan ASC");
                      }
                        $no = 1;
                          while ($r=mysql_fetch_array($tampil)){
